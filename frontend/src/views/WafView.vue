...
const search = ref('')
const results = ref<{ nome: string }[]>([])
const error = ref('')
const loading = ref(false)
const congrats32 = ref(false)
...
async function buscar() {
  ...
  loading.value = true
  try {
    const res = await fetch(`/api/waf?search=${encodeURIComponent(search.value)}`)
    const data = await res.json()
    if (!res.ok) {
      error.value = data.error ?? 'Erro ao pesquisar.'
    } else {
      results.value = data
      if (data.some((r: { nome: string }) => r.nome?.includes('JUMENTOS{'))) {
        congrats32.value = true
      }
    }
  } catch {
...
<template>
  <div class="site">
    <div v-if="congrats32" class="congrats-banner congrats-32">
      <div class="congrats-body">
        <span class="congrats-label">LEVEL 3-2 COMPLETE</span>
        <span class="congrats-msg">Contornaste o WAF através de duplo-aninhamento de palavras-chave. O filtro de 2 passagens foi insuficiente.</span>
        <RouterLink to="/secrets" class="congrats-next">Avançar para Nível 3-3 &rarr;</RouterLink>
      </div>
      <span class="congrats-dismiss" @click="congrats32 = false">&#x2715;</span>
    </div>
    ...
    <nav class="navbar">
      ...
      <ul class="nav-links">
        ...
        <li><RouterLink to="/waf" class="nav-active">WAF</RouterLink></li>
        ...
        <table v-if="results.length" class="entries-table">
          ...
          <tbody>
            <tr v-for="r in results" :key="r.nome">
              <td :class="{ 'flag-row': r.nome?.includes('JUMENTOS{') }">{{ r.nome }}</td>
            </tr>
          </tbody>
        </table>
      </div>
...
.flag-row { color: #C9A84C; font-family: monospace; font-weight: 700; }
